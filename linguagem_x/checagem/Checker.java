...
public class Checker extends Visitor {
    ...
    /**
     * Dada uma instância de TBase (sintático) retorna uma equivalente do tipo TipoBaseSemantico
     * (semântico).
     *
     * @param b instância de tipo sintático
     * @return instância de tipo semântico
     */
    ...
    /**
     * Dado um TBase (sintático) e uma dimensão, retorna um equivalente do tipo TipoArraySemantico
     * (semântico).
     *
     * @param b tipo base sintático (sintaxe abstrata)
     * @param d dimensão do array
     * @return instância de TipoArraySemantico
     */
     ...
     *
     * @param nivel nível em que está o identificador
     * @return endereço do frame
     */
    ...
    /**
     * Adiciona em um HashMap as referências dos endereços criados para o frame.
     * 
     * @param id nome do identificador
     * @param e endereço do frame
     */
    ...
    /**
     * Recupera o endereço de um identificador no HashMap.
     * 
     * @param  id nome do identificador
     * @return endereço do frame
     */
    ...
    public Object visitAssign(ASSIGN a) {
        ...
        if (tVar.tipo.equals(TipoBaseSemantico.Real) && tExp.equals(TipoBaseSemantico.Int)) {
            a.exp = new IntParaReal(a.exp);
            tExp = (TipoSemantico) a.exp.accept(this);
        }
        if (!tVar.tipo.equals(tExp)) {
            erros.reportar(400, "Não é possível atribuir " + tExp + " em " + tVar.tipo);
        }
        return tVar.tipo;
    }
    ...
    public Object visitBinExp(BinExp e) {
        ...
        tEsq =
                (esq instanceof VinculavelConsVar)
                        ? ((VinculavelConsVar) esq).tipo
                        : (TipoSemantico) esq;
        tDir =
                (dir instanceof VinculavelConsVar)
        ...
        switch (e.operacao.token) {
            ...
            case "/":
            case "%":
                if (tEsq instanceof TipoArraySemantico || tDir instanceof TipoArraySemantico) {
    ...
    public Object visitChamada(CHAMADA c) {
        ...
        if (vinculo == null) {
            erros.reportar(404, "Procedimento " + c.id + " Não encontrado no " + "ambiente!");
        } else {
            if (vinculo.isFunc) {
                erros.reportar(
                        001,
                        "O identificador chamado é uma função,"
                                + " utilize CHAMADA apenas com procedimentos.");
            }
            if (c.listaExp.size() != vinculo.paramProc.size()) {
                erros.reportar(
                        002,
                        "O número de parâmetros da chamada é "
                        ...
                        if (p2 instanceof VinculavelConsVar) {
                            ...
                            erros.reportar(
                                    ...
                                    "O procedimento "
                                            + c.id
                                            + "exige passagem de parâmetros por "
                                            + "referência. Para cham�-lo passe uma "
                                            + "variável.");
                        }
                    }
    ...
    public Object visitChamadaExp(ChamadaExp c) {
        ...
        if (f == null) {
            erros.reportar(404, "função " + c.id + " Não encontrada no " + "ambiente!");
            return TipoBaseSemantico.Int;
        } else {
            if (!f.isFunc) {
                erros.reportar(
                        001,
                        "O identificador chamado é um procedimento,"
    ...
    public Object visitFuncao(Funcao f) {
        if (!aFuncProc.contem(f.id)) {
            ...
            if (!retorno.equals(r2)) {
                erros.reportar(
                        201,
                        "O tipo de retorno da função não é o mesmo retornado pela expressão.");
            }
            aConsVar.terminaEscopo();
    ...
    public Object visitIndexada(Indexada i) {
        ...
        if (t instanceof TipoBaseSemantico) {
            erros.reportar(
                    303,
                    "Não é possível realizar a indexação em"
    ...
    public Object visitMenos(Menos m) {
        ...
        if (t.equals(TipoBaseSemantico.Int) || t.equals(TipoBaseSemantico.Real)) {
            ...
            erros.reportar(101, "operação unária de Menos inválida para " + t);
            return TipoBaseSemantico.Int;
        }
    }
    ...
    public Object visitTipoArray(TipoArray t) {
        TipoArraySemantico s = new TipoArraySemantico(this.paraSemantico(t.base), t.exp.size());
        return s;
    }
    ...
    public Object visitWhile(WHILE w) {
        ...
        if (!t.equals(TipoBaseSemantico.Bool)) {
            erros.reportar(
                    103,
                    "A condição do WHILE deve ser do tipo bool." + " Um " + t + " foi passado.");
        }
